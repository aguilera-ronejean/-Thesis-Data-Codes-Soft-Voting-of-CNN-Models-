...
foldFolder = fullfile('../','FileExchangeEntry/5 Folds (10-24-0.0001-LDF0.1-LDRP16)/');
...
%Plot ALL ROC in ONE Figure
t = tiledlayout(2,2); % Requires R2019b or later
nexttile 
    % ROC Curve - AOM
    [fp_rate,tp_rate,T1,AUC]=perfcurve(test_labels,posterior(:,1),1);
    figure;
    plot(fp_rate,tp_rate,'b-');
    grid on;
    xlabel('False Positive Rate - AOM');
    ylabel('Detection Rate - AOM');
nexttile
    % ROC Curve - CSOM
    [fp_rate2,tp_rate2,T2,AUC2]=perfcurve(test_labels,posterior(:,2),2);
    figure;
    plot(fp_rate2,tp_rate2,'b-');
    grid on;
    xlabel('False Positive Rate - CSOM');
    ylabel('Detection Rate - CSOM');
    % Area under the ROC curve value
    AUC2()
nexttile
    % ROC Curve - Earwax
    ...
    figure;
    plot(fp_rate3,tp_rate3,'b-');
    grid on;
    xlabel('False Positive Rate - Earwax');
    ylabel('Detection Rate - Earwax');
    % Area under the ROC curve value
    AUC3()
nexttile
    % ROC Curve - Normal_Image
    [fp_rate4,tp_rate4,T4,AUC4]=perfcurve(test_labels,posterior(:,4),4);
    figure;
    plot(fp_rate4,tp_rate4,'b-');
    grid on;
    xlabel('False Positive Rate - Normal');
    ylabel('Detection Rate - Normal');
    % Area under the ROC curve value
    AUC4()
