tic
    ...
    for fold_idx=1:num_folds
       ...
       % Test Indices for current fold
        test_idx_googlenet=fold_idx:num_folds:num_images;
    
        % Test cases for current fold
         imdsTest = subset(imds,test_idx_googlenet);
         labeltest_googlenet=countEachLabel(imdsTest)
        % Train indices for current fold
        train_idx_googlenet=setdiff(1:length(imds.Files),test_idx_googlenet);
        
        % Train cases for current fold
        imdsTrain = subset(imds,train_idx_googlenet);
    
        % Validation Case for Current Fold
        imdsValidation = subset(augmentedImageDatastore([224 224],imds),train_idx_googlenet);
        
        labeltrain_googlenet= countEachLabel(imdsTrain)
       ...
       % Resizing all training images to [224 224] for Googlenet architecture
        ...
        % Training
        netTransfer_googlenet = trainNetwork(auimds,lgraph,options);
        
        % Resizing all testing images to [224 224] for Googlenet architecture   
         augtestimds = augmentedImageDatastore([224 224],imdsTest);
       
        % Testing and their corresponding Labels and posterior_googlenet for each Case
        [predicted_labels_googlenet(test_idx_googlenet),posterior_googlenet(test_idx_googlenet,:)] = classify(netTransfer_googlenet, augtestimds)
        % Save the Independent ResNet Architectures obtained for each Fold
        save(sprintf('GOOGLENET_custom6_%d_among_%d_folds',fold_idx,num_folds),'netTransfer_googlenet','test_idx_googlenet','train_idx_googlenet','labeltest_googlenet','labeltrain_googlenet', 'predicted_labels_googlenet', "posterior_googlenet");
        
        % Clearing unnecessary variables 
        clearvars -except fold_idx num_folds num_images predicted_labels_googlenet posterior_googlenet imds netTransfer_googlenet net numClasses;
        
    end
    allImages = augmentedImageDatastore([227 227],imds);
    analyzeNetwork(netTransfer_googlenet);
    %%Performance Study
    % Actual Labels
    ...
    % Confusion Matrix
    figure;
    plotconfusion(actual_labels,predicted_labels_googlenet')
    clear title;
    ...
    figure;
    [fp_rate,tp_rate,T,AUC]=perfcurve(test_labels,posterior_googlenet(:,1),1);
    plot(fp_rate,tp_rate,'LineWidth',1.5)
    hold on
    [fp_rate2,tp_rate2,T2,AUC2]=perfcurve(test_labels,posterior_googlenet(:,2),2);
    plot(fp_rate2,tp_rate2,'LineWidth',1.5)
    [fp_rate3,tp_rate3,T3,AUC3]=perfcurve(test_labels,posterior_googlenet(:,3),3);
    plot(fp_rate3,tp_rate3,'LineWidth',1.5)
    [fp_rate4,tp_rate4,T4,AUC4]=perfcurve(test_labels,posterior_googlenet(:,4),4);
    ...
    PREDICTED=predicted_labels_googlenet';
    ...
    PREDICTED2=predicted_labels_googlenet';
    idx2 = (ACTUAL2()=='csom');
    %disp(idx)
    ...
    PREDICTED3=predicted_labels_googlenet';
    idx3 = (ACTUAL3()=='aom');
    %disp(idx)
    ...
    PREDICTED4=predicted_labels_googlenet';
    idx4 = (ACTUAL4()=='earwax');
    %disp(idx)
    ...
    PREDICTED_normal=predicted_labels_googlenet';
    ...
    xlswrite(filename,title_cat,'Sheet1','A83'); %Always change to empty cell
    xlswrite(filename,VALUES,'Sheet1', 'A84'); %Always change to empty cell
    %winopen(filename);  
    %end
    %winopen(filename);
toc
