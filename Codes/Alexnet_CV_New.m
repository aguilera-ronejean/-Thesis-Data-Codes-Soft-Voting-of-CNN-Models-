tic
    outputFolder = fullfile('../','dataset_augmented_undersampled_histeq/');
    ...
    for fold_idx=1:num_folds
       ...
       % Test Indices for current fold
        test_idx_alex=fold_idx:num_folds:num_images;
    
        % Test cases for current fold
         imdsTest = subset(imds,test_idx_alex);
         labeltest_alex=countEachLabel(imdsTest)
        % Train indices for current fold
        train_idx_alex=setdiff(1:length(imds.Files),test_idx_alex);
        
        % Train cases for current fold
        imdsTrain = subset(imds,train_idx_alex);

        % Validation Case for Current Fold
        imdsValidation = subset(augmentedImageDatastore([227 227],imds),test_idx_alex);
        
        labeltrain_alex= countEachLabel(imdsTrain)
    ...
    layers = [
       ...
       % Resizing all training images to [227 227] for Alexnet architecture
        ...
        % Training
        netTransfer_alex = trainNetwork(auimds,layers,options);
        
        % Resizing all testing images to [227 227] for Alexnet architecture   
         augtestimds = augmentedImageDatastore([227 227],imdsTest);
       
        % Testing and their corresponding Labels and posterior_alex for each Case
        [predicted_labels_alex(test_idx_alex),posterior_alex(test_idx_alex,:)] = classify(netTransfer_alex, augtestimds)
        % Save the Independent ResNet Architectures obtained for each Fold
        save(sprintf('TEST47ALEXNETcustom6_%d_among_%d_folds',fold_idx,num_folds),'netTransfer_alex','test_idx_alex','train_idx_alex','labeltest_alex','labeltrain_alex', 'predicted_labels_alex', "posterior_alex");
        
        % Clearing unnecessary variables 
        clearvars -except fold_idx num_folds num_images predicted_labels_alex posterior_alex imds netTransfer_alex net numClasses;
        
    end
    allImages = augmentedImageDatastore([227 227],imds);
    analyzeNetwork(netTransfer_alex);
    %%Performance Study
    % Actual Labels
    ...
    % Confusion Matrix
    figure;
    plotconfusion(actual_labels,predicted_labels_alex')
    clear title;
    ...
    figure;
    [fp_rate,tp_rate,T,AUC]=perfcurve(test_labels,posterior_alex(:,1),1);
    plot(fp_rate,tp_rate,'LineWidth',1.5)
    hold on
    [fp_rate2,tp_rate2,T2,AUC2]=perfcurve(test_labels,posterior_alex(:,2),2);
    plot(fp_rate2,tp_rate2,'LineWidth',1.5)
    [fp_rate3,tp_rate3,T3,AUC3]=perfcurve(test_labels,posterior_alex(:,3),3);
    plot(fp_rate3,tp_rate3,'LineWidth',1.5)
    [fp_rate4,tp_rate4,T4,AUC4]=perfcurve(test_labels,posterior_alex(:,4),4);
    ...
    PREDICTED_normal=predicted_labels_alex';
    ...
    xlswrite(filename,title_cat,'Sheet1','A80'); %Always change to empty cell
    xlswrite(filename,VALUES,'Sheet1', 'A81'); %Always change to empty cell
    %winopen(filename);  
    %end
    %winopen(filename);
toc
