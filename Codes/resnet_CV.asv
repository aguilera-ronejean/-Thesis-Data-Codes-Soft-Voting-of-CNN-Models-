tic
    ...
    for fold_idx=1:num_folds
       ...
       % Test Indices for current fold
        resnet_test_idx=fold_idx:num_folds:num_images;
    
        % Test cases for current fold
         imdsTest = subset(imds,resnet_test_idx);
         resnet_labeltest=countEachLabel(imdsTest)
        % Train indices for current fold
        resnet_train_idx=setdiff(1:length(imds.Files),resnet_test_idx);
        
        % Train cases for current fold
        imdsTrain = subset(imds,resnet_train_idx);
    
        imdsValidation = subset(augmentedImageDatastore([224 224],imds),resnet_test_idx);
        
        resnet_labeltrain= countEachLabel(imdsTrain)
       ...
       % Resizing all training images to [224 224] for Googlenet architecture
        ...
        % Training
        netTransfer_resnet = trainNetwork(auimds,lgraph,options);
        
        % Resizing all testing images to [224 224] for Googlenet architecture   
         augtestimds = augmentedImageDatastore([224 224],imdsTest);
       
        % Testing and their corresponding Labels and resnet_posterior for each Case
        [resnet_predicted_labels(resnet_test_idx),resnet_posterior(resnet_test_idx,:)] = classify(netTransfer_resnet, augtestimds)
        % Save the Independent ResNet Architectures obtained for each Fold
        save(sprintf('RESNET_TEST8_%d_among_%d_folds',fold_idx,num_folds),'netTransfer_resnet','resnet_test_idx','resnet_train_idx','resnet_labeltest','resnet_labeltrain', 'resnet_predicted_labels', "resnet_posterior");
        
        % Clearing unnecessary variables 
        clearvars -except fold_idx num_folds num_images resnet_predicted_labels resnet_posterior imds netTransfer_resnet net numClasses;
        
    end
    allImages = augmentedImageDatastore([224 224],imds);
    analyzeNetwork(netTransfer_resnet);
    %%Performance Study
    % Actual Labels
    ...
    % Confusion Matrix
    figure;
    plotconfusion(actual_labels,resnet_predicted_labels')
    clear title;
    ...
    figure;
    [fp_rate,tp_rate,T,AUC]=perfcurve(test_labels,resnet_posterior(:,1),1);
    plot(fp_rate,tp_rate,'LineWidth',1.5)
    hold on
    [fp_rate2,tp_rate2,T2,AUC2]=perfcurve(test_labels,resnet_posterior(:,2),2);
    plot(fp_rate2,tp_rate2,'LineWidth',1.5)
    [fp_rate3,tp_rate3,T3,AUC3]=perfcurve(test_labels,resnet_posterior(:,3),3);
    plot(fp_rate3,tp_rate3,'LineWidth',1.5)
    [fp_rate4,tp_rate4,T4,AUC4]=perfcurve(test_labels,resnet_posterior(:,4),4);
    ...
    PREDICTED=resnet_predicted_labels';
    ...
    PREDICTED2=resnet_predicted_labels';
    ...
    PREDICTED3=resnet_predicted_labels';
    ...
    PREDICTED4=resnet_predicted_labels';
    ...
    PREDICTED_normal=resnet_predicted_labels';
    ...
    xlswrite(filename,title_cat,'Sheet1','A100'); %Always change to empty cell
    xlswrite(filename,VALUES,'Sheet1', 'A101'); %Always change to empty cell
    %winopen(filename);  
    %end
    %winopen(filename);
toc
